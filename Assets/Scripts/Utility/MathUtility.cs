...
public static class MathUtility {
  public static bool LineSegmentsIntersect(Vector2 a1, Vector2 a2, Vector2 b1,
                                           Vector2 b2) {
  ...
  public static bool LinesIntersect(Vector2 a1, Vector2 a2, Vector2 a3,
                                    Vector2 a4) {
    return (a1.x - a2.x) * (a3.y - a4.y) - (a1.y - a2.y) * (a3.x - a4.x) != 0;
  }

  public static Vector2 PointOfLineLineIntersection(Vector2 a1, Vector2 a2,
                                                    Vector2 a3, Vector2 a4) {
  ...
  public static float DistanceFromPointToLine(Vector2 a, Vector2 b, Vector2 p) {
    var s1 = -b.y + a.y;
    var s2 = b.x - a.x;
    return Mathf.Abs((p.x - a.x) * s1 + (p.y - a.y) * s2) /
           Mathf.Sqrt(s1 * s1 + s2 * s2);
  }

  /// Pseudo distance of point C from the line passing through points A and B.
  /// This is not the actual distance value, but a further point will always
  /// have a higher value than a nearer point. Faster than calculating the
  /// actual distance. Useful for sorting.
  public static float PseudoDistanceFromPointToLine(Vector2 a, Vector2 b,
                                                    Vector2 p) {
    return Mathf.Abs((p.x - a.x) * (-b.y + a.y) + (p.y - a.y) * (b.x - a.x));
  }

  public static Vector2 ClosestPointOnLineSegment(Vector2 a, Vector2 b,
                                                  Vector2 p) {
    var aB = b - a;
    var aP = p - a;
  ...
  public static Vector3 ClosestPointOnLineSegment(Vector3 a, Vector3 b,
                                                  Vector3 p) {
    var aB = b - a;
    var aP = p - a;
    var sqrLenAb = aB.sqrMagnitude;
  ...
  public static int SideOfLine(Vector2 a, Vector2 b, Vector2 p) {
    return (int)Mathf.Sign((p.x - a.x) * (-b.y + a.y) +
                           (p.y - a.y) * (b.x - a.x));
  }
  ...
  public static bool PointInTriangle(Vector2 a, Vector2 b, Vector2 c,
                                     Vector2 p) {
    var area = 0.5f * (-b.y * c.x + a.y * (-b.x + c.x) + a.x * (b.y - c.y) +
                       b.x * c.y);
    var s = 1 / (2 * area) *
  ...
  public static bool RaySphere(Vector3 centre, float radius, Vector3 rayOrigin,
                               Vector3 rayDir, out Vector3 intersectionPoint) {
    // See:
    // http://viclw17.github.io/2018/07/16/raytracing-ray-sphere-intersection/
    var offset = rayOrigin - centre;
    var a = Vector3.Dot(
        rayDir, rayDir); // sqr ray length (in case of non-normalized rayDir)
    ...
    if (discriminant >= 0) {
      ...
      // float t2 = (-b + Mathf.Sqrt (discriminant)) / (2 * a); // The further
      // away intersection point

      // If t is negative, the intersection was in negative ray direction so
      // ignore it
      if (t >= 0) {
  ...
  public static Vector2 V3ToXZ(Vector3 v3) { return new Vector2(v3.x, v3.z); }
