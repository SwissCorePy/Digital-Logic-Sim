...
public class ComponentEditBounds : MonoBehaviour {
    ...
    private void UpdateSignalAreaSizeAndPos(Transform signalArea) {
        ...
        signalArea.localScale =
            new Vector3(signalArea.localScale.x, transform1.localScale.y, 1);
    }
    ...
    private void CreateMatrices() {
        ...
        Vector3[] edgeCentres = { centre + Vector3.left * width / 2,
                                  centre + Vector3.right * width / 2,
                                  centre + Vector3.up * height / 2,
                                  centre + Vector3.down * height / 2
                                };

        Vector3[] edgeScales = { new Vector3(thickness, height + thickness, 1),
        ...
        for (var i = 0; i < 4; i++)
            _trs[i] =
                Matrix4x4.TRS(edgeCentres[i], Quaternion.identity, edgeScales[i]);
    }
}
}
