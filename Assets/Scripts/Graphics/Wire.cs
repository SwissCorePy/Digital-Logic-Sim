...
public class Wire : MonoBehaviour {
  ...
  public Pin ChipOutputPin =>
      startPin.pinType == Pin.PinType.ChipOutput ? startPin : endPin;
  ...
  private void LateUpdate() {
    ...
    _lineRenderer.startWidth =
        (_selected ? selectedThickness : thickness) * ThicknessMultiplier;
    _lineRenderer.endWidth =
        (_selected ? selectedThickness : thickness) * ThicknessMultiplier;
  }

  public void TellWireSimIsOff() { simActive = false; }

  public void tellWireSimIsOn() { simActive = true; }
  ...
  private void UpdateWirePos() {
    ...
    var endPointError = (Vector2)endPin.transform.position -
  ...
  public void ConnectToFirstPinViaWire(Pin pin, Wire parentWire,
                                       Vector2 inputPoint) {
    _lineRenderer = GetComponent<LineRenderer>();
    _mat = simpleMat;
    _drawPoints = new List<Vector2>();
