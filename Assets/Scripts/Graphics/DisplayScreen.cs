...
public class DisplayScreen : BuiltinChip {
  ...
  private static Texture2D CreateSolidTexture2D(Color color, int width,
                                                int height = -1) {
    if (height == -1)
      height = width;
  ...
  protected override void ProcessOutput() {
    ...
    _texture.SetPixel(
        ...
        new Color(Convert.ToInt32(
                      inputPins[0].State + inputPins[1].State.ToString(), 2) /
                      2f,
                  Convert.ToInt32(
                      inputPins[2].State + inputPins[3].State.ToString(), 2) /
                      2f,
                  Convert.ToInt32(
                      inputPins[4].State + inputPins[5].State.ToString(), 2)) /
            2f);
    _texture.Apply();
  }
}
