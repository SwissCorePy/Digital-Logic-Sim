...
public class PinAndWireInteraction : InteractionHandler {
  ...
  private void HandlePinHighlighting() {
    var mousePos = InputHelper.MouseWorldPos;
    var pinCollider = Physics2D.OverlapCircle(
  ...
  private void DeleteChipWires(Chip.Chip chip) {
    ...
    for (var i = 0; i < wiresToDestroy.Count; i++)
  ...
  private enum State { None, PlacingWire }
}
}
