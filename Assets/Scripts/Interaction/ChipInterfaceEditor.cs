...
public class ChipInterfaceEditor : InteractionHandler {
  ...
  private const int maxGroupSize = 16;
  ...
  public ChipEditor CurrentEditor { set => currentEditorName = value.chipName; }
  ...
  private void SetGroupSize(int groupSize) { currentGroupSize = groupSize; }
    ...
    if (HasFocus) {
      ...
      if (highlightedSignal == null && !isDragging)
        if (mouseInInputBounds) {
          ...
          else if (InputHelper.AnyOfTheseKeysDown(
                       KeyCode.Minus, KeyCode.KeypadMinus, KeyCode.Underscore))
            currentGroupSize =
                Mathf.Clamp(currentGroupSize - 1, 1, maxGroupSize);
  ...
  private void HandleSpawning() {
    ...
    if (Input.GetMouseButtonDown(0)) {
      ...
      for (var i = 0; i < currentGroupSize; i++) {
        ...
        var spawnPos = new Vector3(containerX, posY,
                                   chipContainer.position.z + forwardDepth);

        var spawnedSignal = Instantiate(signalPrefab, spawnPos,
                                        Quaternion.identity, signalHolder);
        if (isGroup) {
  ...
  private void HidePreviews() {
    for (var i = 0; i < previewSignals.Length; i++)
      previewSignals[i].gameObject.SetActive(false);
  }

  private float ClampY(float y) {
    return Mathf.Clamp(y, BoundsBottom + handleSize.y / 2f,
                       BoundsTop - handleSize.y / 2f);
  }
  ...
  private ChipSignal GetSignalUnderMouse() {
    ...
    for (var i = 0; i < signals.Count; i++) {
      ...
      var insideX = mousePos.x >= handleCentre.x - halfSizeX &&
                    mousePos.x <= handleCentre.x + halfSizeX;
      var insideY = mousePos.y >= handleCentre.y - halfSizeY &&
  ...
  private void DrawHandle(float y,
    ...
    UnityEngine.Graphics.DrawMesh(quadMesh, handleMatrix, currentHandleMat, 0);
  }
