...
public class ChipInteraction : InteractionHandler {
    ...
    public List<Chip.Chip> allChips {
        get;
        private set;
    }
    ...
    private void Awake() {
        ...
        editChipMenu =
            GameObject.Find("Edit Chip Menu").GetComponent<EditChipMenu>();
        editChipMenu.Init();
    }
    ...
    private void HandleSelectionBox() {
        ...
        if (Input.GetMouseButton(0)) {
            var pos =
                (Vector3)(_selectionBoxStartPos + mousePos) / 2 + Vector3.back * 0.5f;
            var scale =
                new Vector3(Mathf.Abs(mousePos.x - _selectionBoxStartPos.x),
                            Mathf.Abs(mousePos.y - _selectionBoxStartPos.y), 1);
            selectionBoxMaterial.color = selectionBoxCol;
            UnityEngine.Graphics.DrawMesh(
                _selectionMesh, Matrix4x4.TRS(pos, Quaternion.identity, scale),
        ...
        var unused =
            Physics2D.OverlapBoxNonAlloc((_selectionBoxStartPos + mousePos) / 2,
                                         boxSize, 0, allObjectsInBox, chipMask);
        _selectedChips.Clear();
    ...
    private void HandleChipMovement() {
        ...
        if (Input.GetMouseButton(0)) {
            ...
            for (var i = 0; i < _selectedChips.Count; i++) {
                _selectedChips[i].transform.position =
        ...
        if (Input.GetMouseButtonUp(0)) {
            ...
            if (SelectedChipsWithinPlacementArea()) {
                ...
                if (_selectedChips.Count > 1 &&
                    ...
                    var objectUnderMouse = InputHelper.GetObjectUnderMouse2D(chipMask);
                    if (!objectUnderMouse?.GetComponent<Chip.Chip>())
                        return;

                    _selectedChips.Clear();
                    if (objectUnderMouse is {})
                        _selectedChips.Add(objectUnderMouse.GetComponent<Chip.Chip>());
                } else {
                    for (var i = 0; i < _selectedChips.Count; i++)
    ...
    private void PlaceNewChips() {
        var startDepth = allChips.Count > 0
                         ? allChips[allChips.Count - 1].transform.position.z
                         : 0;
        for (var i = 0; i < _newChipsToPlace.Count; i++)
            SetDepth(_newChipsToPlace[i],
                     startDepth + (_newChipsToPlace.Count - i) * ChipDepth);
    ...
    private void CancelPlacement() {
        for (var i = _newChipsToPlace.Count - 1; i >= 0; i--)
            Destroy(_newChipsToPlace[i].gameObject);
        _newChipsToPlace.Clear();
        _selectedChips.Clear();
        _currentState = State.None;
    }
    ...
    protected override bool CanReleaseFocus() {
        return _currentState != State.PlacingNewChips &&
               _currentState != State.MovingOldChips;
    }
    ...
    private enum State { None, PlacingNewChips, MovingOldChips, SelectingChips }
}
}
