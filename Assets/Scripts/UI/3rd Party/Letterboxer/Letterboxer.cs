...
namespace UI._3rd_Party.Letterboxer {
  ...
  public class Letterboxer : MonoBehaviour {
    ...
    private void HandleMaintainAspectRatio() {
      ...
      Camera.rect = scaleHeight < 1.0f ? GetLetterboxRect(scaleHeight)
                                       : GetPillarboxRect(scaleHeight);
    }
    ...
    private void HandleBestFit() {
      ...
      var yHeightFactor =
          targetHeight * scaleFactor / (float)currentScreenHeight;

      Camera.rect =
          new Rect(GetRectPosition(xWidthFactor, currentScreenWidth),
                   GetRectPosition(yHeightFactor, currentScreenHeight),
                   xWidthFactor, yHeightFactor);
    }
    ...
    private float GetOffset(int size) { return size % 2 == 0 ? 0 : 1f / size; }
    ...
    private bool ShouldUpdateLetterbox() {
      return Screen.width != currentScreenWidth ||
