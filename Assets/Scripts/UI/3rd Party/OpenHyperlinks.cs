...
namespace UI._3rd_Party {
  ...
  public class OpenHyperlinks : MonoBehaviour, IPointerClickHandler {
    ...
    protected virtual void Awake() {
      ...
      // Get a reference to the camera if Canvas Render Mode is not ScreenSpace
      // Overlay.
    ...
    private void LateUpdate() {
      // is the cursor in the correct region (above the text area) and
      // furthermore, in the link region?
      var isHoveringOver = TMP_TextUtilities.IsIntersectingRectTransform(
          pTextMeshPro.rectTransform, Input.mousePosition, pCamera);
      var linkIndex = isHoveringOver
                          ? TMP_TextUtilities.FindIntersectingLink(
                                pTextMeshPro, Input.mousePosition, pCamera)
                          : -1;

      // Clear previous link selection if one existed.
      if (pCurrentLink != -1 && linkIndex != pCurrentLink) {
        // Debug.Log("Clear old selection");
        SetLinkToColor(pCurrentLink,
                       (linkIdx, vertIdx) =>
                           pOriginalVertexColors[linkIdx][vertIdx]);
      ...
      if (linkIndex != -1 && linkIndex != pCurrentLink) {
        ...
        if (doesColorChangeOnHover)
          pOriginalVertexColors =
              SetLinkToColor(linkIndex, (_linkIdx, _vertIdx) => hoverColor);
      }

      // Debug.Log(string.Format("isHovering: {0}, link: {1}", isHoveringOver,
      // linkIndex));
    }

    public void OnPointerClick(PointerEventData eventData) {
      // Debug.Log("Click at POS: " + eventData.position + "  World POS: " +
      ...
      if (linkIndex != -1) {
        ...
        // Debug.Log(string.Format("id: {0}, text: {1}", linkInfo.GetLinkID(),
        // linkInfo.GetLinkText())); open the link id as a url, which is the
        // metadata we added in the text field
        Application.OpenURL(linkInfo.GetLinkID());
      }
    }

    private List<Color32[]>
    SetLinkToColor(int linkIndex, Func<int, int, Color32> colorForLinkAndVert) {
      var linkInfo = pTextMeshPro.textInfo.linkInfo[linkIndex];

      var oldVertColors =
          new List<Color32[]>(); // store the old character colors

      for (var i = 0; i < linkInfo.linkTextLength; i++) {
        // for each character in the link string
        var characterIndex = linkInfo.linkTextfirstCharacterIndex +
                             i; // the character index into the entire text
        ...
        if (charInfo.isVisible) {
          vertexColors[vertexIndex + 0] =
              colorForLinkAndVert(i, vertexIndex + 0);
          vertexColors[vertexIndex + 1] =
              colorForLinkAndVert(i, vertexIndex + 1);
          vertexColors[vertexIndex + 2] =
              colorForLinkAndVert(i, vertexIndex + 2);
          vertexColors[vertexIndex + 3] =
              colorForLinkAndVert(i, vertexIndex + 3);
        }
      }
