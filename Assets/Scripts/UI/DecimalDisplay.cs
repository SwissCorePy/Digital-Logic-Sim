...
public class DecimalDisplay : MonoBehaviour
{
    ...
    {
        foreach (var signalGroup in _displayGroups) signalGroup.UpdateDisplay(_signalEditor);
    }

    private void RebuildGroups()
    {
        foreach (var signalGroup in _displayGroups) Destroy(signalGroup.text.gameObject);
        ...
        {
            if (!group[0].displayGroupDecimalValue) continue;
    ...
    private class SignalGroup
    {
        ...
        public void UpdateDisplay(ChipInterfaceEditor editor)
        {
            ...
            else
            {
                ...
                var yPos = (signals[0].transform.position.y + signals[signals.Length - 1].transform.position.y) /
                           2f;
