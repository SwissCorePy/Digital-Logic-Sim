...
public static class ChipLoader {
  public static SavedChip[] GetAllSavedChips(string[] chipPaths) {
    ...
    for (var i = 0; i < chipPaths.Length; i++) {
      ...
      // If the save does not contain wireType and contains outputPinNames then
      // its a previous version save (v 0.25 expected)
  ...
  // Instantiates all components that make up the given clip, and connects them
  // up with wires The components are parented under a single "holder" object,
  // which is returned from the function
  private static ChipSaveData
  LoadChip(SavedChip chipToLoad,
           Dictionary<string, Chip.Chip> previouslyLoadedChips) {
    ...
    for (var i = 0; i < numComponents; i++) {
      ...
      if (!previouslyLoadedChips.ContainsKey(componentName))
        Debug.LogError("Failed to load sub component: " + componentName +
                       " While loading " + chipToLoad.name);

      var loadedComponentChip = Object.Instantiate(
      ...
      for (var outputIndex = 0; outputIndex < componentToLoad.outputPins.Length;
           outputIndex++) {
    ...
    for (var chipIndex = 0; chipIndex < chipToLoad.savedComponentChips.Length;
         chipIndex++) {
      ...
      for (var inputPinIndex = 0;
        ...
        if (savedPin.parentChipIndex != -1) {
          ...
          if (Pin.TryConnect(connectedPin, pin)) {
            var loadedWire =
                Object.Instantiate(wirePrefab, chipEditor.wireHolder);
  ...
  public static ChipSaveData GetChipSaveData(Chip.Chip chip,
                                             Chip.Chip[] builtinChips,
                                             List<Chip.Chip> spawnableChips,
                                             Wire wirePrefab,
                                             ChipEditor chipEditor) {
    // @NOTE: chipEditor can be removed here if:
    //     * Chip & wire instantiation is inside their respective implementation
    ...
    // Maintain dictionary of loaded chips (initially just the built-in chips)
    var loadedChips = new Dictionary<string, Chip.Chip>();
    foreach (var builtinChip in builtinChips)
      loadedChips.Add(builtinChip.chipName, builtinChip);
    ...
    var wireLayout = LoadWiringFile(
        SaveSystem.GetPathToWireSaveFile(loadedChipData.chipName));
    ...
    foreach (var wire in wireLayout.serializableWires) {
      var startPinName = loadedChipData.componentChips[wire.parentChipIndex]
      ...
      var lenght2 =
          loadedChipData.componentChips[wire.childChipIndex].inputPins.Length;

      Debug.Log("2. Index: " + element2Index + " for lenght " + lenght2);

      var unused2 = loadedChipData.componentChips[wire.childChipIndex]
                        .inputPins[wire.childChipInputIndex];

      var endPinName = loadedChipData.componentChips[wire.childChipIndex]
                           .inputPins[wire.childChipInputIndex]
                           .pinName;
  ...
  public static void Import(string path) {
    ...
    using var reader = new StreamReader(path);
    var numberOfChips =
        int.Parse(reader.ReadLine() ?? throw new InvalidOperationException());

    for (var i = 0; i < numberOfChips; i++) {
      var chipName = reader.ReadLine();
      var saveDataLength =
          int.Parse(reader.ReadLine() ?? throw new InvalidOperationException());
      var wireSaveDataLength =
          int.Parse(reader.ReadLine() ?? throw new InvalidOperationException());

      var saveData = "";
      var wireSaveData = "";

      for (var j = 0; j < saveDataLength; j++)
        saveData += reader.ReadLine() + "\n";
      for (var j = 0; j < wireSaveDataLength; j++)
      ...
      if (Array.FindIndex(allChips, c => c.name == chipName) >= 0) {
        ...
        nameUpdateLookupTable.Add(
            chipName ?? throw new InvalidOperationException(), newName);
        chipName = newName;
      }
