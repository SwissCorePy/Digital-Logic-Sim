...
public class Manager : MonoBehaviour {
    ...
    public void ViewChip(Chip.Chip chip) {
        ...
        var chipSaveData = ChipLoader.GetChipSaveData(
                               chip, builtinChips, spawnableChips, wirePrefab, _activeChipEditor);
    ...
    private Chip.Chip TryPackageAndReplaceChip(string original) {
        var oldPackage = Array.Find(GetComponentsInChildren<ChipPackage>(true),
                                    cp => cp.name == original);
        if (oldPackage != null)
