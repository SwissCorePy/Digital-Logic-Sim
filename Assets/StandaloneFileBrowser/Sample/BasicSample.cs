...
public class BasicSample : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Open File Async"))
            StandaloneFileBrowser.OpenFilePanelAsync("Open File", "", "", false, paths => {
            WriteResult(paths);
        });
        GUILayout.Space(5);
        ...
        if (GUILayout.Button("Open Folder Async"))
            StandaloneFileBrowser.OpenFolderPanelAsync("Select Folder", "", true, paths => {
            WriteResult(paths);
        });
        GUILayout.Space(5);
        ...
        if (GUILayout.Button("Save File Async"))
            StandaloneFileBrowser.SaveFilePanelAsync("Save File", "", "", "", path => {
            WriteResult(path);
        });
        GUILayout.Space(5);
