...
namespace StandaloneFileBrowser.Sample {
  public class BasicSample : MonoBehaviour {
    ...
    private void OnGUI() {
      var guiScale =
          new Vector3(Screen.width / 800.0f, Screen.height / 600.0f, 1.0f);
      ...
      if (GUILayout.Button("Open File Async"))
        StandaloneFileBrowser.OpenFilePanelAsync(
            "Open File", "", "", false, paths => { WriteResult(paths); });
      GUILayout.Space(5);
      if (GUILayout.Button("Open File Multiple"))
        WriteResult(
            StandaloneFileBrowser.OpenFilePanel("Open File", "", "", true));
      GUILayout.Space(5);
      if (GUILayout.Button("Open File Extension"))
        WriteResult(
            StandaloneFileBrowser.OpenFilePanel("Open File", "", "txt", true));
      GUILayout.Space(5);
      if (GUILayout.Button("Open File Directory"))
        WriteResult(StandaloneFileBrowser.OpenFilePanel(
            "Open File", Application.dataPath, "", true));
      GUILayout.Space(5);
      if (GUILayout.Button("Open File Filter")) {
        ...
        WriteResult(StandaloneFileBrowser.OpenFilePanel("Open File", "",
                                                        extensions, true));
      }
      ...
      if (GUILayout.Button("Open Folder")) {
        var paths =
            StandaloneFileBrowser.OpenFolderPanel("Select Folder", "", true);
        WriteResult(paths);
      }
      ...
      if (GUILayout.Button("Save File Async"))
        StandaloneFileBrowser.SaveFilePanelAsync(
            "Save File", "", "", "", path => { WriteResult(path); });
      GUILayout.Space(5);
      ...
      if (GUILayout.Button("Save File Directory"))
        _path = StandaloneFileBrowser.SaveFilePanel(
            "Save File", Application.dataPath, "", "");
      GUILayout.Space(5);
      if (GUILayout.Button("Save File Filter")) {
        // Multiple save extension filters with more than one extension support.
        var extensionList = new[] { new ExtensionFilter("Binary", "bin"),
                                    new ExtensionFilter("Text", "txt") };
        _path = StandaloneFileBrowser.SaveFilePanel(
            "Save File", "", "MySaveFile", extensionList);
      }
    ...
    public void WriteResult(string path) { _path = path; }
  }
}